...
#randomize walls
specials = [(dimension-1, 1, "red", -1), (dimension-1, 0, "green", 1)]
specialsL = [[dimension-1, 1], [dimension-1, 0]]
walls=[]
walls=np.random.randint(dimension,size=(int((dimension*dimension)/10),2))
walls=walls.tolist()

for s in specialsL:
    if( s in walls):
        walls = walls.remove( s)
